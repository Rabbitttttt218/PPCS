...
# 加了个聚类
# 在它下面，新增这个函数：
def save_checkpoint_with_step(base_model, optimizer, epoch, step, total_steps, metrics, best_metrics, ckpt_name, args, logger=None):
    # 这个函数与 save_checkpoint 几乎一样，只是多保存了 step 和 total_steps
    if args.local_rank == 0:
        state = {
            'base_model': base_model.module.state_dict() if args.distributed else base_model.state_dict(),
            'optimizer': optimizer.state_dict(),
            'epoch': epoch,
            'step': step,               # 新增
            'total_steps': total_steps, # 新增
            'metrics': metrics.state_dict(),
            'best_metrics': best_metrics.state_dict()
        }
        path = os.path.join(args.experiment_path, 'checkpoints')
        if not os.path.exists(path):
            os.makedirs(path)
        
        torch.save(state, os.path.join(path, f'{ckpt_name}.pth'))
        print_log(f"Save checkpoint at step {step} to {os.path.join(path, f'{ckpt_name}.pth')}", logger=logger)
