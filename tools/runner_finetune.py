...
# # class Acc_Metric:
# #     def __init__(self, acc=0., mean_class_acc=0.):
# #         if type(acc).__name__ == 'dict':
# #             self.acc = acc['acc']
# #             self.mean_class_acc = acc.get('mean_class_acc', 0.)
# #         elif type(acc).__name__ == 'Acc_Metric':
# #             self.acc = acc.acc
# #             self.mean_class_acc = getattr(acc, 'mean_class_acc', 0.)
# #         else:
# #             self.acc = acc
# #             self.mean_class_acc = mean_class_acc
...
# #         _dict = dict()
# #         _dict['acc'] = self.acc
# #         _dict['mean_class_acc'] = self.mean_class_acc
# #         return _dict
...
# #         # 计算每个类别的准确率
# #         cls_num = config.model.cls_dim
# #         class_acc = torch.zeros(cls_num)
# #         class_count = torch.zeros(cls_num)
        
# #         for i in range(test_label.size(0)):
# #             class_count[test_label[i]] += 1
# #             if test_pred[i] == test_label[i]:
# #                 class_acc[test_label[i]] += 1
        
# #         # 防止除以零
# #         valid_class_count = class_count > 0
# #         class_acc[valid_class_count] = class_acc[valid_class_count] / class_count[valid_class_count] * 100.
        
# #         # 输出总体准确率和各类别准确率
# #         print_log('[Validation] EPOCH: %d  acc = %.4f' % (epoch, acc), logger=logger)
        
# #         # 计算有效类别的平均准确率
# #         valid_class_acc = class_acc[valid_class_count]
# #         mean_class_acc = valid_class_acc.mean().item()
        
# #         print_log('[Validation] EPOCH: %d  mean_class_acc = %.4f' % (epoch, mean_class_acc), logger=logger)
        
# #         # 输出每个类别的准确率
# #         for i in range(cls_num):
# #             if class_count[i] > 0:
# #                 print_log('[Validation] EPOCH: %d  Class %d: acc = %.4f, count = %d' % 
# #                           (epoch, i, class_acc[i], int(class_count[i])), logger=logger)
...
# #     if val_writer is not None:
# #         val_writer.add_scalar('Metric/ACC', acc, epoch)
# #         val_writer.add_scalar('Metric/Mean_Class_ACC', mean_class_acc, epoch)
...
# #         print_log('[Validation_vote] EPOCH: %d  mean_class_acc_vote = %.4f' % (epoch, mean_class_acc), logger=logger)
        
# #         # 输出每个类别的准确率
# #         for i in range(cls_num):
# #             if class_count[i] > 0:
# #                 print_log('[Validation_vote] EPOCH: %d  Class %d: acc_vote = %.4f, count = %d' % 
# #                           (epoch, i, class_acc[i], int(class_count[i])), logger=logger)
...
# #     if val_writer is not None:
# #         val_writer.add_scalar('Metric/ACC_vote', acc, epoch)
# #         val_writer.add_scalar('Metric/Mean_Class_ACC_vote', mean_class_acc, epoch)
...
# #         print_log('[TEST] mean_class_acc = %.4f' % mean_class_acc, logger=logger)
        
# #         # 输出每个类别的准确率
# #         for i in range(cls_num):
# #             if class_count[i] > 0:
# #                 print_log('[TEST] Class %d: acc = %.4f, count = %d' % 
# #                           (i, class_acc[i], int(class_count[i])), logger=logger)
...
# #             acc = 0.
# #             mean_class_acc = 0.
# #             best_class_accs = torch.zeros(cls_num)

# #             for time in range(1, 300):
# #                 this_acc, this_mean_class_acc, this_class_accs = test_vote(base_model, test_dataloader, 1, None, args, config, logger=logger, times=10)
# #                 if acc < this_acc:
# #                     acc = this_acc
# #                     mean_class_acc = this_mean_class_acc
# #                     best_class_accs = this_class_accs
                
# #                 print_log('[TEST_VOTE_time %d] acc = %.4f, mean_class_acc = %.4f, best acc = %.4f, best mean_class_acc = %.4f' % 
# #                          (time, this_acc, this_mean_class_acc, acc, mean_class_acc), logger=logger)
            
# #             print_log('[TEST_VOTE] acc = %.4f, mean_class_acc = %.4f' % (acc, mean_class_acc), logger=logger)
            
# #             # 输出每个类别的最佳准确率
# #             for i in range(cls_num):
# #                 if class_count[i] > 0:
# #                     print_log('[TEST_VOTE] Class %d: best_acc = %.4f, count = %d' % 
# #                             (i, best_class_accs[i], int(class_count[i])), logger=logger)
...
# #     # print_log('[TEST] acc = %.4f' % acc, logger=logger)
# #     return acc, mean_class_acc, class_acc
...
# #             # 检查data是否包含双路径数据
# #             if len(data) == 3 and isinstance(data[2], torch.Tensor) and config.model.path_strategy == 'both':
# #                 # 双路径数据处理
# #                 points = data[0].cuda()
# #                 label = data[1].cuda()
# #                 trans_points = data[2].cuda()
...
# #                 # 对trans-zigzag路径执行相同的FPS采样
# #                 if trans_points.size(1) < point_all:
# #                     point_all = trans_points.size(1)
                    
# #                 fps_idx_trans = pointnet2_utils.furthest_point_sample(trans_points, point_all)  # (B, npoint)
# #                 fps_idx_trans = fps_idx_trans[:, np.random.choice(point_all, npoints, False)]
# #                 trans_points = pointnet2_utils.gather_operation(trans_points.transpose(1, 2).contiguous(), fps_idx_trans).transpose(1, 2).contiguous()  # (B, N, 3)
                
# #                 # 应用数据增强
# #                 if 'scan' in args.config:
# #                     points = train_transforms(points)
# #                     trans_points = train_transforms(trans_points)
# #                 else:
# #                     points = train_transforms_raw(points)
# #                     trans_points = train_transforms_raw(trans_points)
                    
# #                 # 将双路径数据传递给模型
# #                 ret = base_model((taxonomy_ids, model_ids, points, trans_points))
...
# #     with torch.no_grad():
# #         for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
# #             # 处理双路径数据的方式取决于测试策略 - 通常测试时只使用zigzag_only
# #             if len(data) == 3 and isinstance(data[2], torch.Tensor) and config.model.path_strategy == 'both':
# #                 # 在测试时仍然可能使用指定的策略，这里我们保持灵活性
# #                 points = data[0].cuda()
# #                 label = data[1].cuda()
# #                 trans_points = data[2].cuda()
...
# #                 # 进行FPS采样
# #                 points = misc.fps(points, npoints)
# #                 trans_points = misc.fps(trans_points, npoints)
                
# #                 # 向模型传递双路径数据
# #                 logits = base_model((taxonomy_ids, model_ids, points, trans_points))
# #             else:
# #                 # 单路径数据处理
# #                 points = data[0].cuda()
# #                 label = data[1].cuda()
...
# #                 if points_raw.size(1) < point_all:
# #                     point_all = points_raw.size(1)
# #                 if trans_points_raw.size(1) < point_all:
# #                     point_all_trans = trans_points_raw.size(1)
# #                 else:
# #                     point_all_trans = point_all
                
# #                 # 对两种路径分别进行FPS采样
# #                 fps_idx_raw = pointnet2_utils.furthest_point_sample(points_raw, point_all)
# #                 fps_idx_trans_raw = pointnet2_utils.furthest_point_sample(trans_points_raw, point_all_trans)
...
# #                     # 处理trans-zigzag路径
# #                     fps_idx_trans = fps_idx_trans_raw[:, np.random.choice(point_all_trans, npoints, False)]
# #                     trans_points = pointnet2_utils.gather_operation(trans_points_raw.transpose(1, 2).contiguous(),
# #                                                                fps_idx_trans).transpose(1, 2).contiguous()
# #                     trans_points = test_transforms(trans_points)
...
# #                 # 获取特征
# #                 _, concat_f = base_model((taxonomy_ids, model_ids, points, trans_points))
# #             else:
# #                 # 单路径处理
# #                 points = data[0].cuda()
# #                 label = data[1].cuda()
...
#         # 添加早期检查点保存功能 - 在每个epoch结束后调用
#         if hasattr(args, 'save_early_checkpoints') and args.save_early_checkpoints and hasattr(args, 'save_early_checkpoint_func'):
#             # 获取实际的模型（去除DataParallel/DistributedDataParallel包装）
#             if hasattr(base_model, 'module'):
#                 model_to_save = base_model.module
#             else:
#                 model_to_save = base_model
            
#             args.save_early_checkpoint_func(
#                 model=model_to_save,
#                 optimizer=optimizer,
#                 epoch=epoch,
#                 total_epochs=config.max_epoch,
#                 exp_name=args.exp_name,
#                 progress_points=args.early_checkpoint_percentages,
#                 logger=logger
#             )
