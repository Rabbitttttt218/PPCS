...
# # 聚类2
# /home/dlss107552304043/PointMamba/main.py (完整、最终修改版)

import datetime
import random
import argparse
# 使用您项目中的正确文件名
from tools import runner_pretrain as pretrain
from tools import runner_finetune as finetune
from tools import test_run_net as test_net
...
def get_args():
    ...
    # === 新增参数 ===
    parser.add_argument(
        '--early_checkpoint_ratios', type=float, nargs='+',
        default=[0.1, 0.2],
        help='Training progress ratios to save checkpoints (e.g., 0.1 for 10%)'
    )
    # === 新增结束 ===
...
def main():
    ...
    # === 新增代码 ===
    # 将早期检查点比率传递给config，以便runner文件可以使用
    config.early_checkpoint_ratios = args.early_checkpoint_ratios
    # === 新增结束 ===

    # --- 您所有的原始 batch size 设置逻辑保持不变 ---
    if args.distributed:
        assert config.total_bs % args.world_size == 0
        if config.dataset.get('train'):
            config.dataset.train.others.bs = config.total_bs // args.world_size
    ...
    if args.test:
        ...
        # 确保调用 runner 文件中的 run_net 函数
        if args.finetune_model or args.scratch_model:
            finetune.run_net(args, config, train_writer, val_writer)
        else:
            pretrain.run_net(args, config, train_writer, val_writer)
